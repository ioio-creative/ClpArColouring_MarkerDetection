...
//#include "Marker.hpp"  // moved to MarkerDetector.hpp, by Christopher Wong, 2017-10-03
#include "TinyLA.hpp"
#include "DebugHelpers.hpp"

//MarkerDetector::MarkerDetector(CameraCalibration calibration)
MarkerDetector::MarkerDetector(const int refMarkers[4][5][5])
	: m_minContourLengthAllowed(100)
	, markerSize(100, 100)
{
	// We are not using CameraCalibration for our application
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 5; j++)
		{
			for (int k = 0; k < 5; k++)
			{
				m_refMarkersToTest[i][j][k] = refMarkers[i][j][k];
			}
		}
	}
}
...
//bool MarkerDetector::processFrame(const cv::Mat& cameraFrame)
void MarkerDetector::processFrame(const cv::Mat& cameraFrame, bool isMarkersPresent[4],
	std::vector<cv::Point2f>& markerCentres)
{
	//bool isMarkerPresent = false;

	std::vector<Marker> markers;
	findMarkers(cameraFrame, markers, isMarkersPresent, false);  // false means use absolute thresholding
	
	// if not all 4 markers are present, findMarkers() again with adaptive thresholding, last arg set to true
	if (!(isMarkersPresent[0] && isMarkersPresent[1] && isMarkersPresent[2] && isMarkersPresent[3]))
	{
		findMarkers(cameraFrame, markers, isMarkersPresent, true);
	}

	// if 4 all markers are present
	if (isMarkersPresent[0] && isMarkersPresent[1] && isMarkersPresent[2] && isMarkersPresent[3])
	{
		// I have to ensure that markerCentres are in the order:
		// top-left, top-right, bottom-left, bottom-right	
		// This is achieved by the nested for loops as follows		
		for (int i = 0; i < 4; i++)
		{
			for (Marker marker : markers)
			{
				if (marker.matchedRefMarkerIdx == i)
				{
					markerCentres.push_back(marker.getMarkerCentre());
					break;  // TODO: Is it ok to break here? Presumably yes.
				}
			}
		}
	}
	...
	// as it may depend on the results of estimatePosition().
	// But I keep it here for later reference.
	...
	//isMarkerPresent = m_transformations.size() > 0;
...
{
	// No need to do conversion from BGRAVideoFrame to cv::Mat
	// as the input is changed from type BGRAVideo to cv::Mat
	...
	// Make it binary
	if (isUseAdaptiveThresholding)
	{
		performThresholdAdaptive(m_grayscaleImage, m_thresholdImg);
	}
	else
	{
		performThresholdAbsolute(m_grayscaleImage, m_thresholdImg);
	}
	...
	// Find if they are markers
	recognizeMarkers(m_grayscaleImage, detectedMarkers, isMarkersPresent);

	// Calculate their poses
	// Can't calculate pose as we haven't input CameraCalibration
...
void MarkerDetector::prepareImage(const cv::Mat& bgraMat, cv::Mat& grayscale) const
{
	...
	cv::showAndSave("Grayscale.png", grayscale);
#endif
	...
	pFilter = cv::createCLAHE(5.0, cv::Size(8, 8));
	pFilter->apply(grayscale, grayscale);*/
	
//#if SHOW_DEBUG_IMAGES
//	cv::showAndSave("CLAHE_ed.png", grayscale);
//#endif
}

void MarkerDetector::performThresholdAbsolute(const cv::Mat& grayscale, cv::Mat& thresholdImg) const
{
	// Use either Absolute Threshold or Adaptive Threshold
	// Book p. 84
	// The Book said Adaptive Threshold (averaging involved) is preferred.
	// But I find Absolute Threshold gives better results during test.

	//cv::threshold(grayscale, thresholdImg, 125, 255, cv::THRESH_BINARY_INV | cv::THRESH_OTSU);

	cv::threshold(grayscale, thresholdImg, 127, 255, cv::THRESH_BINARY_INV);

#if SHOW_DEBUG_IMAGES
	cv::showAndSave("Threshold_absolute image.png", thresholdImg);
#endif
}

void MarkerDetector::performThresholdAdaptive(const cv::Mat& grayscale, cv::Mat& thresholdImg) const
{
...
#if SHOW_DEBUG_IMAGES
	cv::showAndSave("Threshold_adaptive image.png", thresholdImg);
#endif
}
...
void MarkerDetector::findContours(cv::Mat& thresholdImg, ContoursVector& contours, int minContourPointsAllowed) const
{
...
#if SHOW_DEBUG_IMAGES
	{
		...
		cv::showAndSave("Contours.png", contoursImage);
	}
#endif
}
	...
	bool isMarkersPresent[4])
{
	...
	// initialize isMarkersPresent
	for (int i = 0; i < noOfMarkersToTest; i++)
	{
		isMarkersPresent[i] = false;
	}
...
//#ifdef SHOW_DEBUG_IMAGES
#if SHOW_DEBUG_IMAGES
		{
			...
			cv::showAndSave("Source marker " + ToString(i) + ".png", markerSubImage);
			cv::showAndSave("Marker " + ToString(i) + " after warp.png", canonicalMarkerImage);
		}
#endif

		// The following is commented out
		// Because now I have to test for presence of multiple markers in input image
		...
		// Added by Christopher Wong
		// Test for presence of multiple markers in input image

		int nRotations[4] = { 0, 0, 0, 0 }; // integer array to store orientation of marker present in input image
		int* ids = Marker::getMarkerIds(canonicalMarkerImage, m_refMarkersToTest, nRotations);
		for (int j = 0; j < noOfMarkersToTest; j++)
		{
			if (ids[j] != -1)
			{
				marker.id = ids[j];
				marker.matchedRefMarkerIdx = j;

				//sort the points so that they are always in the same order no matter the camera orientation
				std::rotate(marker.points.begin(), marker.points.begin() + 4 - nRotations[j], marker.points.end());

				goodMarkers.push_back(marker);

				isMarkersPresent[j] = true;
	...
	// The following process of "Marker location refinement" (p. 74 or p. 93 according to Edge)
	// of textbook) may not be required for our purpose
	
	// Actually, I changed my mind.
	// I may require the positions of the 4 markers for perspective transform.
	// Hence, the following location refinement is needed.
...
//		}
//
//		cv::showAndSave("Markers refined edges.png", grayscale * 0.5 + markerCornersMat);
//	}
//#endif
...
bool MarkerDetector::processFrameToDetectOneMarker(const cv::Mat& frame, const int markerToDetect[5][5], 
	std::vector<Marker>& detectedMarkers)
{
	bool isMarkerPresent = false;

	isMarkerPresent = detectOneMarker(frame, markerToDetect, detectedMarkers, false);  // false means use absolute thresholding

	// if marker is not present, detectOneMarker() again with adaptive thresholding, last arg set to true
	if (!isMarkerPresent)
	{
		detectOneMarker(frame, markerToDetect, detectedMarkers, true);
	}
...
// Added by Christopher Wong, 2017-10-03
bool MarkerDetector::detectOneMarker(const cv::Mat& frame, const int markerToDetect[5][5], 
	std::vector<Marker>& detectedMarkers, bool isUseAdaptiveThresholding)
{
	...
	// Find if they are markers
	isMarkerPresent = recognizeOneMarker(m_grayscaleImage, markerToDetect,
		detectedMarkers);
...
// Added by Christopher Wong, 2017-10-03
bool MarkerDetector::recognizeOneMarker(const cv::Mat& grayscale, const int markerToDetect[5][5], 
	std::vector<Marker>& detectedMarkers)
{
	std::vector<Marker> goodMarkers;
	...
	for (size_t i = 0; i<detectedMarkers.size(); i++)
	{
		...
		int nRotations;
		int id = Marker::getMarkerIdWithInputMarker(canonicalMarkerImage, markerToDetect, nRotations);
		if (id != -1)
		{
			marker.id = id;
			//sort the points so that they are always in the same order no matter the camera orientation
